...
from DWT.DWT import noise_trigger,merge_N_HH,merge_gray_HH,noise_trigger_gray,merge_N_HL,merge_gray_HL
def get_model(opt):
    netC = None
    optimizerC = None
    schedulerC = None
...
def train(netC, optimizerC, schedulerC, train_dl,  identity_image, tf_writer, epoch, opt):
    print(" Train:")
    netC.train()
    rate_bd = opt.pc
    total_loss_ce = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        for i in range(num_bd):
            inputs_bd[i]= merge_gray_HH(inputs_bd[i].unsqueeze(0),identity_image.unsqueeze(0))
        if opt.attack_mode == "all2one":
            targets_bd = torch.ones_like(targets[:num_bd]) * opt.target_label
        if opt.attack_mode == "all2all":
            targets_bd = torch.remainder(targets[:num_bd]+1, opt.num_classes)
        inputs_cross=inputs[num_bd : (num_bd + num_cross)]
        for i in range(num_cross):
            noise_image = skimage.util.random_noise(identity_image, mode='gaussian', seed=None, clip=True,mean=0,var=0.003)
            noise_image=torch.from_numpy(noise_image).unsqueeze(0).float()
            #torchvision.utils.save_image(noise_grid1, './back_tigger_noise.jpg')
            inputs_cross[i]= merge_gray_HH(inputs_cross[i].unsqueeze(0),noise_image)
        ...
        with torch.no_grad():
            ...
            if opt.cross_ratio:
                ...
                for i in range(bs):
                    noise_image = skimage.util.random_noise(identity_image, mode='gaussian', seed=None, clip=True,mean=0,var=0.003)
                    noise_image = torch.from_numpy(noise_image).unsqueeze(0).float()
                    #torchvision.utils.save_image(noise_grid1, './back_tigger_noise.png')
                    inputs_cross[i] = merge_gray_HH(inputs_cross[i].unsqueeze(0), noise_image)
                inputs_cross=nomalizer(inputs_cross)
...
def main():
    ...
    if opt.continue_training:
        if os.path.exists(opt.ckpt_path):
            ...
            identity_image = state_dict["identity_image"]
    ...
    for epoch in range(epoch_current, opt.n_iters):
        ...
        train(netC, optimizerC, schedulerC, train_dl,  identity_image, tf_writer, epoch, opt)
